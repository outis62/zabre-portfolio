 ...
 return (
    <main className="min-h-screen py-16 xl:py-24">
      <div className="container mx-auto px-6 xl:px-0">
        ...
        <Link
          href="/services"
          className="inline-flex items-center gap-2 text-white/50 hover:text-accent transition-colors duration-300 mb-12 group"
        >
        ...
        <div className="flex flex-col xl:flex-row xl:items-end xl:justify-between gap-6 mb-16 border-b border-white/10 pb-16">
          <div>
            ...
            <p className="text-accent text-lg mt-3">{service.tagline}</p>
          </div>

          <Link
            href="/contact"
            className="self-start xl:self-auto inline-flex items-center gap-3 bg-accent text-primary font-semibold px-8 py-4 rounded-full hover:bg-accent/90 transition-colors duration-300 whitespace-nowrap"
          >
            Start a project
              ...
              <ul className="flex flex-col gap-4">
                {service.deliverables.map((item, i) => (
                  <li key={i} className="flex items-start gap-4">
                    <span className="mt-1 w-5 h-5 rounded-full border border-accent flex-shrink-0 flex items-center justify-center">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-3">
              {service.stack.map((tech, i) => (
                <span
                  key={i}
                  className="px-5 py-2 border border-white/10 rounded-full text-white/70 text-sm hover:border-accent hover:text-accent transition-colors duration-300"
                >
                  {tech}
                </span>
              ))}
          ...
          <div>
            {prev && (
              <Link
                ...
                className="flex items-center gap-3 text-white/40 hover:text-accent transition-colors duration-300 group"
              >
                <BsArrowLeft className="group-hover:-translate-x-1 transition-transform duration-300" />
